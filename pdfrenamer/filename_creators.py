...
AllowedTags = {
    ...
    "{AZ}": " \t\t=\t Last name of first and last authors (separated by comma)",
...
def build_filename(infos, format=None, tags=None):
    ...
    if any(
        ...
        if author_info:
            ...
            if lastnames:
                rep_dict["{AZ}"] = " ".join([lastnames[0], lastnames[-1]])
                ...
                if firstnames:
                    ...
                    rep_dict["{aAetal}"] = firstinitial_lastnames[0] + (
                        " et al." if len(firstinitial_lastnames) > 1 else ""
                    )
                else:
