...
def rename(target, format=None, tags=None):
    ...
    if (
        not tags
    ):  # If tags is a valid variable, it means the format was already checked earlier (i.e. this call to the function rename was generated by
        # an earlier call to the function rename. By not checking again the format, we save time
        tags = check_format_is_valid(format)
        if (
            tags == None
        ):  # if the function check_format_is_valid has returned, then the format is not valid and the function terminates
            return None
        ...
        try:
            ...
            if result["metadata"] and result["identifier"]:
                ...
                ext = os.path.splitext(filename)[
                    -1
                ].lower()  # Extract the file extension from the old file name
...
def add_abbreviations(path_abbreviation_file):
    ...
    try:
        ...
        with open(
            path_UserDefinedAbbreviations, "r"
        ) as UserDefinedAbbreviations_oldfile:
            UserDefinedAbbreviations_old = UserDefinedAbbreviations_oldfile.read()
        with open(
            path_UserDefinedAbbreviations, "w"
        ) as UserDefinedAbbreviations_newfile:
...
def main():
    ...
    # Setup logging
    config.set(
        "verbose", not (args.decrease_verbose)
    )  # store the desired verbose level in the global config of pdf-renamer. This will also automatically update the pdf2bib and pdf2doi logger level.
    logger = logging.getLogger("pdf-renamer")
    ...
    if isinstance(args.case, str) and (
        args.case in ["camel", "snake", "kebab", "none"]
    ):
    ...
    if (
        target == ""
    ):  # This occurs either if the user forgot to add a target, or if the user used the -sd command to set default values
        return
    ## END
    ...
    if (
        results == None
    ):  # This typically happens when target is neither a valid file nor a valid directory. In this case we stop
        return  # the script execution here. Proper error messages were raised by the rename function

    if os.path.isdir(target):
        target = os.path.join(
            target, ""
        )  # This makes sure that, if target is a path to a directory, it has the ending "/" or "\"
    MainPath = os.path.dirname(
        target
    )  # Extract the path of target. If target is a directory, then MainPath = target
    ...
    for result in results:
        if result and result["identifier"] and result["path_new"]:
            if not (result["path_original"] == result["path_new"]):
                print(
                    Fore.YELLOW + f"{os.path.relpath(result['path_original'],MainPath)}"
                )
                print(
                    Fore.MAGENTA
